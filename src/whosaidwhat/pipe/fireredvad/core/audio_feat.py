...
#maduo add
def load_audio_first_channel_16k(path):
    # 读取音频
    audio, sr = sf.read(path)  # audio is float32
    ...
    audio_int16 = np.round(audio_int16 * 32768).astype(np.int16)
...
class AudioFeat:
    ...
    def extract(self, audio):
        if isinstance(audio, str):
            wav_np, sample_rate = load_audio_first_channel_16k(audio)
