...
class MSDDDiarizer:
    def __init__(self, args, device: Union[str, torch.device]):
        self.model: NeuralDiarizer = NeuralDiarizer(cfg=create_config(args)).to(device)
    ...
    def diarize(self, audio: torch.Tensor):
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            for label in pred_labels_clus:
                ...
                print(f"raw start: {start}, raw end : {end}, raw speaker: {speaker}")
                start, end = float(start), float(end)
                start, end = int(start * 1000), int(end * 1000)
                print(f"*1000 start: {start}, *1000 end: {end}")
...
def create_config(args):
    ...
    config.diarizer.msdd_model.model_path = (
        args.msdd_model_path  # Telephonic speaker diarization model, default `diar_msdd_telephonic`
    )
    print(f"config.diarizer.msdd_model.model_path: {config.diarizer.msdd_model.model_path}")
    config.diarizer.vad.parameters.use_speaker_model_from_ckpt=args.use_speaker_model_from_ckpt # default is True
    return config
